...
# External ALB -> Envoy ASG (egress)
resource "aws_security_group_rule" "external_lb_to_envoy_asg_egress" {
  type                     = "egress"
  from_port                = 8081
  to_port                  = 8081
  protocol                 = "tcp"
  security_group_id        = var.external_alb_security_group_id
  source_security_group_id = aws_security_group.envoy_sg.id
  description              = "Health check to Envoy instances"
}

# Envoy ASG <- External ALB (ingress)
resource "aws_security_group_rule" "envoy_asg_from_external_lb_ingress" {
  type                     = "ingress"
  from_port                = 8081
  to_port                  = 8081
  protocol                 = "tcp"
  security_group_id        = aws_security_group.envoy_sg.id
  source_security_group_id = var.external_alb_security_group_id
  description              = "Health check from External LB"
}
